...
@section('content')
    ...
    @if ($pendingEntries->isNotEmpty())
        <div
            ...
            <div>
                <p style="font-size: 0.8rem; font-weight: 600; color: #92400e;">Pending invoice charges for HBL#
                    {{ $pendingHouseBL }}</p>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.25rem;">
            ...
            <div class="card">
                ...
                <div style="display: flex; gap: 8px; margin-top: 0.75rem;">
                    <input type="text" id="search-input" placeholder="Search consignee or House BL..." class="form-input"
                ...
                <div id="search-results"
                    style="display: none; margin-top: 8px; border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden;">
                </div>
            </div>
        ...
        function doSearch() {
            ...
            fetch(`{{ route('invoice.hbl.search') }}?q=${encodeURIComponent(q)}`, {
                ...
                .then(data => {
                    ...
                    if (!data.length) {
                        resultsEl.innerHTML =
                            `<div style="padding: 12px; text-align: center; color: var(--text-muted); font-size: 0.8rem;">No results found.</div>`;
            ...
            fetch('{{ route('invoice.hbl.charges.clear') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('manifest-details').innerHTML =
                            `<div style="padding: 2rem; text-align: center; color: var(--text-muted); font-size: 0.875rem; border: 1.5px dashed var(--border-color); border-radius: 8px;">Search for a consignee or House BL to view manifest details.</div>`;
            ...
            fetch('{{ route('invoice.hbl.store') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.open(`{{ url('invoice/house-bl/report') }}/${encodeURIComponent(hbl)}`,
                                '_blank');
                            // Reset form
                            document.getElementById('charges-panel').style.display = 'none';
                            document.getElementById('manifest-details').innerHTML =
