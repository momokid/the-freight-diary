...
@section('title', 'Other Serv. Invoice')
@section('page-title', 'Other Service Invoice')
...
@section('content')
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem;">
            ...
            <div class="card">
                ...
                <div style="display: flex; gap: 8px; margin-top: 0.75rem; position: relative;">
                    <div style="flex: 1; position: relative;">
                        <input type="text" id="client-search" placeholder="Search Client Name..." class="form-input"
            ...
            <div class="card">
                ...
                <div style="margin-top: 0.75rem; display: flex; flex-direction: column; gap: 0.75rem;">
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <p id="charge-description-error" class="form-error"></p>
                    </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.25rem;">
            ...
            <div class="card">
                <p class="form-title">Recent Client's Activity</p>
                <div id="client-activity" style="margin-top: 0.75rem;">
                    <div
                        ...
                        Search and select a client to view their recent invoices.
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        function searchClient() {
            ...
            fetch(`{{ route('invoice.other-invoice.search-client') }}?q=${encodeURIComponent(q)}`, {
                    headers: {
        ...
        function addCharge() {
            ...
            if (!desc) {
                document.getElementById('charge-description-error').textContent = 'Description is required.';
                document.getElementById('charge-description-error').classList.add('visible');
                valid = false;
            }
            if (!valid) return;
            ...
            fetch('{{ route('invoice.other-invoice.charges.add') }}', {
                    method: 'POST',
                    headers: {
        ...
        function renderChargesTable(entries) {
            ...
            if (!entries.length) {
                wrapper.innerHTML =
                    `<div style="padding: 1.5rem; text-align: center; color: var(--text-muted); font-size: 0.875rem; border: 1.5px dashed var(--border-color); border-radius: 8px;">No charges added yet.</div>`;
                return;
            }
        ...
        function removeCharge(accountNo) {
            fetch('{{ route('invoice.other-invoice.charges.remove') }}', {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CSRF,
        ...
        function clearCharges() {
            ...
            fetch('{{ route('invoice.other-invoice.charges.clear') }}', {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CSRF,
        ...
        function saveInvoice() {
            ...
            const invoiceDesc = 'Service Charge Invoice for ' + document.getElementById('client-search').value;
            ...
            fetch('{{ route('invoice.other-invoice.store') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CSRF,
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.open(
                                `{{ url('invoice/other-invoice/report') }}/${encodeURIComponent(data.ReceiptNo)}`,
                                '_blank');
                            renderChargesTable([]);
                            document.getElementById('client-search').value = '';
                            document.getElementById('client-id').value = '';
