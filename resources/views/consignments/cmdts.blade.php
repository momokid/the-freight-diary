...
@section('content')
    ...
    @if ($pendingContainers->isNotEmpty())
        <div
            ...
            <div>
                <p style="font-size: 0.8rem; font-weight: 600; color: #92400e;">Pending containers for BL#
                    {{ $pendingBOL }}</p>
                <p style="font-size: 0.75rem; color: #92400e; margin-top: 2px;">You have {{ $pendingContainers->count() }}
                    staged container(s). Submit or clear to start a new consignment.</p>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem;">
                <div class="form-group" style="margin-bottom: 0;">
                    <label class="form-label">Select Category <span style="color: #ef4444;">*</span>
                        <button type="button" onclick="openQuickAddModal('category')"
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label class="form-label">Category Type <span style="color: #ef4444;">*</span>
                        <button type="button" onclick="openQuickAddModal('type')"
                            style="margin-left: 6px; color: #16a34a; background: none; border: none; cursor: pointer; font-size: 0.75rem; font-weight: 600;">+
                            New</button>
                    </label>
        ...
        <div class="card">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem; margin-top: 1rem;">
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <input type="text" id="bl" class="form-input" style="text-transform: uppercase;"
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label class="form-label">Shipping Line <span style="color: #ef4444;">*</span>
                        <button type="button" onclick="openQuickAddModal('carrier')"
                            style="margin-left: 6px; color: #16a34a; background: none; border: none; cursor: pointer; font-size: 0.75rem; font-weight: 600;">+
                            New</button>
                    </label>
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label class="form-label">Release Type <span style="color: #ef4444;">*</span>
                        <button type="button" onclick="openQuickAddModal('release')"
                            style="margin-left: 6px; color: #16a34a; background: none; border: none; cursor: pointer; font-size: 0.75rem; font-weight: 600;">+
                            New</button>
                    </label>
                ...
                {{-- Location --}}
                <div class="form-group" style="margin-bottom: 0;">
                    <label class="form-label">Location (Terminal) <span
            ...
            <div style="margin-top: 1.5rem; padding-top: 1.25rem; border-top: 1px solid var(--border-color);">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem;">
                    <div>
                        ...
                        <p id="container-count" style="font-size: 0.75rem; color: var(--text-muted); margin-top: 2px;">No
                            containers added yet</p>
                    </div>
    ...
    <div id="modal-container"
        ...
        <div class="card" style="width: 100%; max-width: 480px; margin: 1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="modal-container-no" class="form-input" style="text-transform: uppercase;"
            ...
            <div class="form-group">
                ...
                <input type="text" id="modal-seal-no" class="form-input" style="text-transform: uppercase;"
                    placeholder="e.g. SEAL123456">
            </div>
            <div class="form-group">
                <label class="form-label">Item Details <span style="color: #ef4444;">*</span></label>
                <textarea id="modal-item-details" rows="3" class="form-input" style="resize: none;"
                    placeholder="Describe the goods in this container..."></textarea>
            ...
            <div style="display: flex; gap: 0.75rem; margin-top: 0.5rem;">
                ...
                <button onclick="addContainer()" id="modal-add-btn" class="btn-primary" style="flex: 1;">Add Container
                    Details</button>
            </div>
    ...
    <div id="modal-quick-type"
        ...
        <div class="card" style="width: 100%; max-width: 400px; margin: 1rem;">
            ...
            <div style="display: flex; gap: 0.75rem;">
                ...
                <button onclick="saveQuickAddType()" id="qam-type-btn" class="btn-primary"
    ...
    <script>
        ...
        function renderContainersTable(containers) {
            ...
            if (!containers.length) {
                wrapper.innerHTML =
                    `<div style="padding: 1.5rem; text-align: center; color: var(--text-muted); font-size: 0.875rem; border: 1.5px dashed var(--border-color); border-radius: 8px;">No containers added yet.</div>`;
                return;
            }
        ...
        function removeContainer(containerNo) {
            fetch('{{ route('cmdts.containers.remove') }}', {
                ...
                .then(res => res.json())
                .then(data => {
                    if (data.success) renderContainersTable(data.containers);
                });
        }
        ...
        function saveQuickAddType() {
            ...
            if (!name) {
                err.textContent = 'Type name is required.';
                err.classList.add('visible');
                return;
            }
            if (!categoryId) {
                err.textContent = 'Please select a category first.';
                err.classList.add('visible');
                return;
            }
            ...
            fetch('{{ route('master-data.commodities.type.store') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        const opt = document.createElement('option');
                        opt.value = data.TypeID;
                        opt.textContent = data.TypeName;
        ...
        function updateSmsPreview() {
            document.getElementById('sms-preview').value =
                `Dear Client, your consignment BL# ${_smsBL} has been registered with PSIL. ` +
